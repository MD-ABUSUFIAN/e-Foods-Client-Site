...
const Dinner = () => {
    ...
    useEffect(()=>{
        setIsLoading(true)
        fetch('https://pacific-journey-19792.herokuapp.com/specific?item=dinner')
        .then(res=>res.json())
        .then(data=>{
          console.log(data)
    ...
    return (
        <Container id="dinner" className="my-5 main-box pb-5">
            { isLoading && <Spinner animation="border" variant="primary" />}
            <h1 className="text-center py-3 fw-bolder text-info fs-1 main-box">Welcome e-Foods Dinner Food Items</h1>
